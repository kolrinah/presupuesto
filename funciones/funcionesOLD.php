...
function get_tabla_presupuesto($id_ac, $id_ae, $id_ue, $year_poa, $id_fuente)
{
	...
	{
		$resulta=queryDB("SELECT presupuestos.id_pgen, presupuestos.id_psub, presupuestos.id_pespe, presupuestos.id_pespe_sub, presupuestos.monto_presup
										FROM presupuestos
									  ...
									  ORDER BY presupuestos.id_pgen, presupuestos.id_psub, presupuestos.id_pespe, presupuestos.id_pespe_sub;");
	}
	
	if ($id_ac==0 && $id_ae==0 && $id_ue==0)
	{
		$resulta=queryDB("SELECT presupuestos.id_pgen, presupuestos.id_psub, presupuestos.id_pespe, presupuestos.id_pespe_sub, presupuestos.monto_presup, un_ejec_locales.s_ext
...
ORDER BY presupuestos.id_pgen, presupuestos.id_psub, presupuestos.id_pespe, presupuestos.id_pespe_sub;");	
	...
	if (pg_num_rows($resulta)>0) 
	{
		...
		while ($row=pg_fetch_assoc($resulta))
		{
			...
			$tabla[$i]['orden']=(pow(10,5)*$row['id_pgen'])+(pow(10,3)*$row['id_psub'])+($row['id_pespe'])+
				(($row['id_pespe_sub']==1000)? 0:(($row['id_pespe_sub'])/100));	
...
function get_tabla_consulta($id_ac, $id_ae, $id_ue, $year_poa, $id_fuente)
{
	if ($id_ac!=0 && $id_ae!=0 && $id_ue!=0)
	{
		$resulta=queryDB("SELECT registros_gastos.id_pgen, registros_gastos.id_psub, registros_gastos.id_pespe, registros_gastos.id_pespe_sub,
													 registros_gastos.fecha_gasto, registros_gastos.monto_gasto
									  ...
									  ORDER BY registros_gastos.id_pgen, registros_gastos.id_psub, registros_gastos.id_pespe, registros_gastos.id_pespe_sub,
										         registros_gastos.fecha_gasto;");
	}
	
	if ($id_ac==0 && $id_ae==0 && $id_ue==0)
	{
		$resulta=queryDB("SELECT registros_gastos.id_pgen, registros_gastos.id_psub, registros_gastos.id_pespe, registros_gastos.id_pespe_sub, registros_gastos.monto_gasto, un_ejec_locales.s_ext, registros_gastos.fecha_gasto
...
function get_matrix($tabla,$presupuesto, $id)  //Obtiene matriz de totalizados por mes
{
	if ($tabla)
	{
		...
		{			
			...
			$matrix[$m]['orden']=(pow(10,5)*$tabla[$i]['id_pgen'])+(pow(10,3)*$tabla[$i]['id_psub'])+($tabla[$i]['id_pespe'])+
				(($tabla[$i]['id_pespe_sub']==1000)? 0:(($tabla[$i]['id_pespe_sub'])/100));	

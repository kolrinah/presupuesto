...
if(isset($_SESSION['nombre']) && isset($_SESSION['id_user']) && isset($_SESSION['email']))
{
	...
	{		
		...
		echo '</a></li>';
		echo '<li><a href="all_gastos.php">Historial Gastos</a></li>';
...
WHERE (((un_ejec_locales.s_ext)='".$_SESSION['s_ext']."') AND ((registros_gastos.fecha_gasto) 
BETWEEN (to_date('".$hora['mday']."/".$hora['mon']."/".$hora['year']."','DD/MM/YYYY')-90) AND to_date('".$hora['mday']."/".$hora['mon']."/".$hora['year']."','DD/MM/YYYY')))
ORDER BY registros_gastos.id_registros_gastos DESC;";

	echo '<div class="titul">Gastos Registrados en los últimos 3 meses</div>';	
